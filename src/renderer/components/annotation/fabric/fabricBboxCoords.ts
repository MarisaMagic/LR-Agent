...
/** @deprecated Use normToSceneRect — kept for callers migrating off display-fit coords */
export function normToFabricRect(
  ann: Pick<BboxAnnotation, 'x' | 'y' | 'width' | 'height'>,
  displayWidth: number,
  displayHeight: number,
): FabricRectPixels {
  return normToSceneRect(ann, displayWidth, displayHeight);
}

/** @deprecated Use sceneRectToNorm */
export function fabricRectToNorm(
  rect: FabricRectPixels,
  displayWidth: number,
  displayHeight: number,
): Pick<BboxAnnotation, 'x' | 'y' | 'width' | 'height'> {
  return sceneRectToNorm(rect, displayWidth, displayHeight);
}
