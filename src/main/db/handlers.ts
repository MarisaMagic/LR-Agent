...
export function registerDbHandlers(): void {
  ...
  ipcMain.handle('db:sessions:getLastMessagePreview', (_event, sessionId: string) => {
  ...
  ipcMain.handle('db:messages:list', (_event, sessionId: string, options: {
  ...
  ipcMain.handle('db:messages:update', (_event, messageId: string, patch: Record<string, unknown>) => {
    return messageRepo.updateMessage(messageId, patch as Parameters<typeof messageRepo.updateMessage>[1]);
  });

  ipcMain.handle('db:messages:deleteAfter', (_event, sessionId: string, sortIndex: number) => {
  ...
  ipcMain.handle('db:messages:deleteAfterId', (_event, sessionId: string, messageId: string) => {
    messageRepo.deleteMessagesAfterId(sessionId, messageId);
  });

  ipcMain.handle('db:messages:cleanupStreaming', (_event, sessionId?: string) => {
  ...
  ipcMain.handle('db:messages:batchCreate', (_event, messages: Array<Parameters<typeof messageRepo.batchCreateMessages>[0][number]>) => {
  ...
  ipcMain.handle('db:providers:create', (_event, provider: {
    id: string;
    name: string;
    baseUrl: string;
  ...
  ipcMain.handle('db:providers:update', (_event, id: string, patch: Record<string, unknown>) => {
    return providerRepo.updateProvider(id, patch as Parameters<typeof providerRepo.updateProvider>[1]);
  });
